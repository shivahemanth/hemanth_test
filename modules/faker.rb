#Generates fake data
#Use this module to generate user specific fake data
 require 'faker'
 ...
 module Fakedata
    ...
    def generate_digits
      Random.new.rand(4000..8000).to_s
    end
    # Generate an email address in the domain `mailinator.com`.
    #This can be helpful to verify emails when registered
    ...
    def generate_phone_number(format = nil)
      case format
        when :paren, :parenthesis, :parentheses
          '(' + NPA.sample + ') ' + NXX.sample + '-' + generate_digits
        when :dot, :dotted, :dots, :period, :periods
          [ NPA.sample, NXX.sample, generate_digits ].join('.')
